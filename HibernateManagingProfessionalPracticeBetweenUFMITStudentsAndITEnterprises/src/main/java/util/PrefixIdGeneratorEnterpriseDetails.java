...
public class PrefixIdGeneratorEnterpriseDetails implements IdentifierGenerator {

    private static final int PREFIX = 2021010258; // Fixed numeric prefix

    @Override
    public Serializable generate(SharedSessionContractImplementor session, Object object) {
        // Get the maximum ID from the database
        Integer maxId = (Integer) session.createQuery("select coalesce(max(id), :prefix) from EnterpriseDetails")
